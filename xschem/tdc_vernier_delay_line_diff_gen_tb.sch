...
"
spice_ignore=false}
C {devices/code.sym} 170 -980 0 0 {name=STIMULI only_toplevel=false value="

.tran 0.0001n 5n
.save v(start) v(stop) v(x1.start_pos) v(x1.start_neg) v(x1.stop_strong)

.control
run
set wr_vecnames
wrdata tdc_vernier_delay_line_diff_gen_tb.txt v(start) v(stop) v(x1.start_pos) v(x1.start_neg) v(x1.stop_strong)
.endc
"}
...
C {devices/vsource.sym} 560 -70 0 0 {name=V2 value="pulse(0 1.8 1ns 0.5ns 0.5ns 0.5ns 2ns)" savecurrent=false}
